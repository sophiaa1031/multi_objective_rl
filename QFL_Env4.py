...
class QFLEnv(gymnasium.Env):
    ...
    def step(self, action):  #
        ...
        for i in range(self.cars):
            ...
            quant[i] = np.log2(1 - pwr[i] * np.log(prob) * 1e7 / np.power(self.rsu_dis[i], 2)) * 32 * bd[
                i]  # *(1000-self.travel_dis[i])/self.vel[i]
        ...
        if self.debug:
            ...
            print("bd", bd)
            print("np.log2(1 + 1e7 * pwr[i] * np.power(self.rsu_dis[i], -2)",
                  np.log2(1 + 1e7 * pwr[i] * np.power(self.rsu_dis[i], -2)))
